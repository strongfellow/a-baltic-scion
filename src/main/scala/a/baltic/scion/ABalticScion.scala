...
import a.baltic.scion.actors.BlockChain
...
class ABalticScion extends Actor {
  ...
  override def preStart(): Unit = {
    ...
    val blockChain = context.actorOf(Props.create(classOf[BlockChain]))
    ...
    val peerConnection = context.actorOf(
        Props.create(classOf[PeerConnection], blockChain, serializer))
